...
// =====================================================
        ...
        // =====================================================
        // 12A. ADVANCE TAX INVOICE OUTSTANDING VALIDATION
        // =====================================================
        /*
         * request.amount is the actual Bank/Cash/Payment Gateway amount.
         * It must not be compared directly with Advance Tax Invoice outstanding
         * because TDS forms part of the settlement.
         *
         * Example:
         * Taxable = ₹100, GST = ₹18, ATI outstanding = ₹118
         * TDS 10% = ₹10, actual bank amount = ₹108
         * Settlement = ₹108 + ₹10 = ₹118
         *
         * FinalPaymentMath validates the exact Bank + TDS settlement later.
         */
        if (existingAdvanceTaxInvoice != null && !isZeroAmountPurchaseOrder) {
            ...
            log.info(
                    "[ADVANCE-INVOICE-OUTSTANDING-VALIDATED] traceId={} | "
                            + "invoiceId={} | invoiceNumber={} | outstanding={} | "
                            + "bankEntered={} | tdsActive={} | tdsPercentage={}",
                    traceId,
                    existingAdvanceTaxInvoice.getId(),
                    existingAdvanceTaxInvoice.getInvoiceNumber(),
        ...
        if (isFirstPayment) {
            ...
            if (existingAdvanceTaxInvoice != null) {
                ...
                // IMPORTANT:
                // When payment is against an existing Advance Tax Invoice,
                // Unbilled must track that ATI's remaining outstanding,
        ...
        log.info(
                "[FINAL-PAYMENT-INPUT] traceId={} | calculationVersion={} | "
                        ...
                        + "outstandingAvailable={} | alreadyUsedTds={}",
                traceId,
                PAYMENT_CALCULATION_VERSION,
                estimate.getId(),
        ...
        private static void validatePaymentType(
            ...
            switch (paymentType) {
                case "FULL" -> {
                    if (settlement.compareTo(outstanding) != 0) {
                        throw fail(
                                "FULL payment must completely settle outstanding",
                ...
                case "PARTIAL" -> {
                    if (settlement.compareTo(BigDecimal.ZERO) <= 0
                            || settlement.compareTo(outstanding) > 0) {
                        throw fail(
                                "PARTIAL settlement must be positive and cannot exceed outstanding",
                ...
                case "INSTALLMENT" -> {
                    if (settlement.compareTo(BigDecimal.ZERO) <= 0
                            || settlement.compareTo(outstanding) > 0) {
                        throw fail(
                                "INSTALLMENT settlement must be positive and cannot exceed outstanding",
                                "ERR_INSTALLMENT_AMOUNT_INVALID",
                                "amount"
                        );
                    }
                }
                ...
                case "PURCHASE_ORDER" -> {
                    if (!purchaseOrderProjectCompleted) {
                        throw fail(
                                "Purchase Order project is not completed",
                                "ERR_PO_PROJECT_NOT_COMPLETED",
                                "paymentTypeId"
                        );
                    }
                    ...
                    if (settlement.compareTo(outstanding) != 0) {
                        throw fail(
                                "Purchase Order payment must completely settle outstanding",
                                "ERR_PURCHASE_ORDER_AMOUNT_MISMATCH",
                                "amount"
                        );
                    }
