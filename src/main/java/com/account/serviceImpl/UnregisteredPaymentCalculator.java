...
/**
 * Handles only normal payment-first UNREGISTERED customer calculations.
 *
 * UNREGISTERED means the customer does not have a GST registration/GSTIN.
 * GST is still calculated from the Estimate because the supplier's taxable
 * supply remains subject to GST.
 *
...
public class UnregisteredPaymentCalculator {
    ...
    private Breakup breakupFromNetBank(
        ...
        if (denominator.compareTo(BigDecimal.ZERO) <= 0) {
            throw fail(
                    ...
                    "ERR_INVALID_UNREGISTERED_TAX_CALCULATION",
                    "tds"
            );
        }
        ...
        switch (paymentType) {
            case PARTIAL -> {
                ...
                BigDecimal expectedPartialAmount =
                        halfAmount.min(outstanding);

                if (settlement.compareTo(expectedPartialAmount) != 0) {
                    throw fail(
                            "PARTIAL payment must be exactly ₹"
                                    + expectedPartialAmount
                                    + ". Total Estimate amount including GST: ₹"
                                    + totalInvoiceAmount
                                    + ", outstanding amount: ₹"
                                    + outstanding
            ...
            default -> throw fail(
                    "Unsupported UNREGISTERED payment type: " + paymentType,
    ...
    private void validateInput(Input input) {
        if (input == null) {
            throw fail(
                    "Unregistered payment calculation input is required",
                    "ERR_UNREGISTERED_PAYMENT_INPUT_REQUIRED",
                    "calculationInput"
            );
        }
        ...
        if (input.getGstRegistrationType()
            ...
            throw fail(
                    "This calculator supports only UNREGISTERED GST",
                    "ERR_UNREGISTERED_GST_TYPE_REQUIRED",
                    "gstRegistrationType"
            );
        }
        ...
        if (input.getActualBankAmount() == null
            ...
            throw fail(
                    ...
                    "ERR_UNREGISTERED_PAYMENT_AMOUNTS_REQUIRED",
                    "amount"
            );
        }
    }
    ...
    private String normalizeType(String code) {
        ...
        if (!FULL.equals(normalized)
            ...
            throw fail(
                    "Unsupported UNREGISTERED payment type: " + normalized,
                    "ERR_UNSUPPORTED_PAYMENT_TYPE",
                    "paymentTypeId"
            );
        }
