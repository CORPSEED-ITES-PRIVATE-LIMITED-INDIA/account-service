...
public interface VoucherRepository extends JpaRepository<Voucher, Long> {
	...
	@Query(value = "SELECT * FROM voucher v WHERE v.product_id =:productId", nativeQuery = true)
	List<Voucher> findAllByProductId(Long productId);
	...
	@Query(value = "SELECT * FROM voucher v WHERE v.ledger_id in (:ledgerList) and v.create_date BETWEEN :d1 AND :d2", nativeQuery = true)
