...
public interface GstDataFromCrmRepository extends JpaRepository<GstDataFromCrm, Long> {
	...
	@Query(value = "SELECT * FROM gst_data_form_crm gd WHERE gd.create_date BETWEEN :d1 AND :d2", nativeQuery = true)
	...
	@Query(value = "SELECT count(*) FROM gst_data_form_crm gd WHERE gd.create_date BETWEEN :d1 AND :d2", nativeQuery = true)
